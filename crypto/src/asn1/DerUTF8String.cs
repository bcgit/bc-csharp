        ...
        : DerStringBase
    {
        ...
        public static DerUtf8String GetInstance(object obj)
        {
            ...
            else if (obj is ArraySegment<byte>)
            {
                try
                {
                    return (DerUtf8String)Meta.Instance.FromByteArray((ArraySegment<byte>)obj);
                }
                catch (IOException e)
                {
                    throw new ArgumentException("failed to construct UTF8 string from ArraySegment<byte>: " + e.Message);
                }
            }
