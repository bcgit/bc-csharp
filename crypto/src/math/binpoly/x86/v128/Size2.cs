...
{
    internal sealed class Size2
        : BinPolyMulBase
    {
        private const int FixedSize = 2;
        private const int FixedSizeExt = 2 * FixedSize;

        internal Size2(int n, IReduce reduce)
            : base(n, reduce)
        {
        ...
        public override void Multiply(ulong[] x, int xOff, ulong[] y, int yOff, ulong[] z, int zOff)
        {
            ...
            Kernels.ImplMul2(x.AsSpan(xOff, FixedSize), y.AsSpan(yOff, FixedSize), tt);
