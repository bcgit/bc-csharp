...
{
    internal sealed class Size1
        : BinPolyMulBase
    {
        private const int FixedSize = 1;
        private const int FixedSizeExt = 2 * FixedSize;

        internal Size1(int n, IReduce reduce)
            : base(n, reduce)
        {
        ...
        public override void Multiply(ulong[] x, int xOff, ulong[] y, int yOff, ulong[] z, int zOff)
        {
            ...
            Kernels.ImplMul1(x.AsSpan(xOff, FixedSize), y.AsSpan(yOff, FixedSize), tt);
