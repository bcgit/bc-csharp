...
namespace Org.BouncyCastle.Crypto.Engines
{
        ...
        : IBlockCipher
    {
        ...
        private readonly uint[] X = new uint[4];
        ...
        public virtual int ProcessBlock(byte[] input, int inOff, byte[] output, int outOff)
        {
            ...
            X[0] = Pack.BE_To_UInt32(input, inOff);
            X[1] = Pack.BE_To_UInt32(input, inOff + 4);
            X[2] = Pack.BE_To_UInt32(input, inOff + 8);
            X[3] = Pack.BE_To_UInt32(input, inOff + 12);
            ...
            Pack.UInt32_To_BE(X[0], output, outOff);
            Pack.UInt32_To_BE(X[1], output, outOff + 4);
            Pack.UInt32_To_BE(X[2], output, outOff + 8);
            Pack.UInt32_To_BE(X[3], output, outOff + 12);
