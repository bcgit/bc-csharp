...
namespace Org.BouncyCastle.Crypto.Tls
{
        ...
        : TlsCipher
    {
        ...
        public virtual byte[] EncodePlaintext(long seqNo, ContentType type, byte[] plaintext, int offset, int len, int outputOffset)
        {
            if (writeMac == null)
            {
                byte[] buffer = new byte[outputOffset + len]; 
                Buffer.BlockCopy(plaintext, offset, buffer, outputOffset, len);
                return buffer;
            }

            byte[] mac = writeMac.CalculateMac(seqNo, type, plaintext, offset, len);
            byte[] ciphertext = new byte[outputOffset + len + mac.Length];
            Buffer.BlockCopy(plaintext, offset, ciphertext, outputOffset, len);
            Buffer.BlockCopy(mac, 0, ciphertext, len, mac.Length);
            return ciphertext;
        }
