...
public class TlsAEADCipher : TlsCipher
{
    ...
    public byte[] EncodePlaintext(long seqNo, ContentType type, byte[] plaintext, int offset, int len, int outputOffset)
    {
        ...
        Buffer.BlockCopy(encryptImplicitNonce, 0, nonce, 0, encryptImplicitNonce.Length);
        ...
        byte[] output = new byte[outputOffset + nonce_explicit_length + ciphertextLength];
        Buffer.BlockCopy(nonce, encryptImplicitNonce.Length, output, outputOffset, nonce_explicit_length);
        int outputPos = outputOffset + nonce_explicit_length;

        byte[] additionalData = GetAdditionalData(seqNo, type, plaintextLength);
        var parameters = new AeadParameters(null, 8 * macSize, nonce, additionalData);
    ...
    protected byte[] GetAdditionalData(long seqNo, ContentType type, int len)        
    {
        /*
        ...
        TlsUtilities.WriteUint8((short)type, additional_data, 8);
