...
namespace Org.BouncyCastle.Crypto.Tests
{
    ...
    {
        private readonly byte[] outBase = Hex.Decode(
        ...
        private readonly byte[] outFFFFFFFF = Hex.Decode(
        ...
        private readonly byte[] outFFFF0000 = Hex.Decode(
        ...
        private readonly byte[] out0000FFFF = Hex.Decode(
        ...
        [Test]
        public void KeyStream_00000000()
        {
            ImplTest(key: Hex.Decode("00000000"), outBase);
        }

        [Test]
        public void KeyStream_FFFFFFFF()
        {
            ImplTest(key: Hex.Decode("FFFFFFFF"), outFFFFFFFF);
        }

        [Test]
        public void KeyStream_Repeating_FFFF0000()
        {
            byte[] key = new byte[256 * 4];
            ...
            ImplTest(key, outFFFF0000);
        }

        [Test]
        public void KeyStream_Repeating_0000FFFF()
        {
            byte[] key = new byte[256 * 4];
            for (int i = 0; i != key.Length; i += 4)
            {
            ...
            ImplTest(key, out0000FFFF);
        }

        private static void ImplTest(byte[] key, byte[] output)
        {
            var engine = new IsaacEngine();
            byte[] input = new byte[output.Length];
            ImplTest(engine, key, input, output);
        }

        private static void ImplTest(IsaacEngine engine, byte[] key, byte[] input, byte[] output)
        {
            byte[] enc = new byte[output.Length];

            engine.Init(true, new KeyParameter(key));
            engine.ProcessBytes(input, 0, input.Length, enc, 0);
            Assert.That(Arrays.AreEqual(output, enc), "ciphertext mismatch");

            engine.Reset();
            engine.ProcessBytes(input, 0, input.Length, enc, 0);
            Assert.That(Arrays.AreEqual(output, enc), "ciphertext mismatch (reset)");

            engine.Init(false, new KeyParameter(key));
            engine.ProcessBytes(output, 0, output.Length, enc, 0);
            Assert.That(Arrays.AreEqual(input, enc), "plaintext mismatch");

            engine.Reset();
            engine.ProcessBytes(output, 0, output.Length, enc, 0);
            Assert.That(Arrays.AreEqual(input, enc), "plaintext mismatch (reset)");
        }
    }
}
