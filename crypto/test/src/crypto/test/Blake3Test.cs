using System;
using System.Text;

using NUnit.Framework;

using Org.BouncyCastle.Crypto.Digests;
using Org.BouncyCastle.Crypto.Parameters;
using Org.BouncyCastle.Utilities;
using Org.BouncyCastle.Utilities.Encoders;
using Org.BouncyCastle.Utilities.Test;

namespace Org.BouncyCastle.Crypto.Tests
{
    [TestFixture]

    /**
     * Test Cases for Blake3.
     * Test Vectors taken from https://github.com/BLAKE3-team/BLAKE3/blob/master/test_vectors/test_vectors.json
     */
    public class Blake3Test
        : SimpleTest
    {
        /**
         * The Blake3 Data Modulus.
         */
        private const int BLAKE3MOD = 251;

        /**
         * The partial output size.
         */
        private const int PARTOUT = 5;

        /**
         * The partial break position.
         */
        private const int PARTBREAK = 100;

        /**
         * The Blake3 Test key.
         */
        private static readonly byte[] BLAKE3KEY = Encoding.ASCII.GetBytes("whats the Elvish word for friend");

        /**
         * The Blake3 Test context.
         */
        private static readonly byte[] BLAKE3CTX =
            Encoding.ASCII.GetBytes("BLAKE3 2019-12-27 16:29:52 test vectors context");

        private readonly TestDef TEST0 = new TestDef(0,
            "",
            "",
            ""
        );
        private readonly TestDef TEST1 = new TestDef(1,
            "",
            "",
            ""
        );
        private readonly TestDef TEST2 = new TestDef(2,
            "",
            "",
            ""
        );
        private readonly TestDef TEST3 = new TestDef(3,
            "",
            "",
            ""
        );
        private readonly TestDef TEST4 = new TestDef(4,
            "",
            "",
            ""
        );
        private readonly TestDef TEST5 = new TestDef(5,
            "",
            "",
            ""
        );
        private readonly TestDef TEST6 = new TestDef(6,
            "",
            "",
            ""
        );
        private readonly TestDef TEST7 = new TestDef(7,
            "",
            "",
            ""
        );
        private readonly TestDef TEST8 = new TestDef(8,
            "",
            "",
            ""
        );
        private readonly TestDef TEST63 = new TestDef(63,
            "",
            "",
            ""
        );
        private readonly TestDef TEST64 = new TestDef(64,
            "",
            "",
            ""
        );
        private readonly TestDef TEST65 = new TestDef(65,
            "",
            "",
            ""
        );
        private readonly TestDef TEST127 = new TestDef(127,
            "",
            "",
            ""
        );
        private readonly TestDef TEST128 = new TestDef(128,
            "",
            "",
            ""
        );
        private readonly TestDef TEST129 = new TestDef(129,
            "",
            "",
            ""
        );
        private readonly TestDef TEST1023 = new TestDef(1023,
            "",
            "",
            ""
        );
        private readonly TestDef TEST1024 = new TestDef(1024,
            "",
            "",
            ""
        );
        private readonly TestDef TEST1025 = new TestDef(1025,
            "",
            "",
            ""
        );
        private readonly TestDef TEST2048 = new TestDef(2048,
            "",
            "",
            ""
        );
        private readonly TestDef TEST2049 = new TestDef(2049,
            "",
            "",
            ""
        );
        private readonly TestDef TEST3072 = new TestDef(3072,
            "",
            "",
            ""
        );
        private readonly TestDef TEST3073 = new TestDef(3073,
            "",
            "",
            ""
        );
        private readonly TestDef TEST4096 = new TestDef(4096,
            "",
            "",
            ""
        );
        private readonly TestDef TEST4097 = new TestDef(4097,
            "",
            "",
            ""
        );
        private readonly TestDef TEST5120 = new TestDef(5120,
            "",
            "",
            ""
        );
        private readonly TestDef TEST5121 = new TestDef(5121,
            "",
            "",
            ""
        );
        private readonly TestDef TEST6144 = new TestDef(6144,
            "",
            "",
            ""
        );
        private readonly TestDef TEST6145 = new TestDef(6145,
            "",
            "",
            ""
        );
        private readonly TestDef TEST7168 = new TestDef(7168,
            "",
            "",
            ""
        );
        private readonly TestDef TEST7169 = new TestDef(7169,
            "",
            "",
            ""
        );
        private readonly TestDef TEST8192 = new TestDef(8192,
            "",
            "",
            ""
        );
        private readonly TestDef TEST8193 = new TestDef(8193,
            "",
            "",
            ""
        );
        private readonly TestDef TEST16384 = new TestDef(16384,
            "",
            "",
            ""
        );
        private readonly TestDef TEST31744 = new TestDef(31744,
            "",
            "",
            ""
        );
        private readonly TestDef TEST102400 = new TestDef(102400,
            "",
            "",
            ""
        );

        public override string Name => "Blake3";

        public override void PerformTest()
        {
            RunTestCase(TEST0);
            RunTestCase(TEST1);
            RunTestCase(TEST2);
            RunTestCase(TEST3);
            RunTestCase(TEST4);
            RunTestCase(TEST5);
            RunTestCase(TEST6);
            RunTestCase(TEST7);
            RunTestCase(TEST8);
            RunTestCase(TEST63);
            RunTestCase(TEST64);
            RunTestCase(TEST65);
            RunTestCase(TEST127);
            RunTestCase(TEST128);
            RunTestCase(TEST129);
            RunTestCase(TEST1023);
            RunTestCase(TEST1024);
            RunTestCase(TEST1025);
            RunTestCase(TEST2048);
            RunTestCase(TEST2049);
            RunTestCase(TEST3072);
            RunTestCase(TEST3073);
            RunTestCase(TEST4096);
            RunTestCase(TEST4097);
            RunTestCase(TEST5120);
            RunTestCase(TEST5121);
            RunTestCase(TEST6144);
            RunTestCase(TEST6145);
            RunTestCase(TEST7168);
            RunTestCase(TEST7169);
            RunTestCase(TEST8192);
            RunTestCase(TEST8193);
            RunTestCase(TEST16384);
            RunTestCase(TEST31744);
            RunTestCase(TEST102400);
        }

        /**
         * Create the blockCipher test suite.
         *
         * @return the test stream
         */
        private void RunTestCase(TestDef pTest)
        {
            /* run tests */
            TestHash(pTest);
            TestKeyed(pTest);
            TestDerived(pTest);
        }

        /**
         * Run hashTest.
         *
         * @param pTest the test
         */
        private void TestHash(TestDef pTest)
        {
            /* Create digest */
            Blake3Digest myDigest = new Blake3Digest();

            /* Try a fake update */
            myDigest.BlockUpdate(BLAKE3KEY, 0, BLAKE3KEY.Length);
            myDigest.Reset();

            /* Save just before final and reset */
            myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
            Blake3Digest mySave = (Blake3Digest)myDigest.Copy();
            myDigest.Reset();

            /* restore save and perform a standard hash */
            myDigest.Reset(mySave);
            byte[] myBase = new byte[myDigest.GetDigestSize()];
            myDigest.DoFinal(myBase, 0);
            byte[] myExpected = Arrays.CopyOf(pTest.theHash, myBase.Length);
            Assert.IsTrue(Arrays.AreEqual(myExpected, myBase), "baseDigest mismatch");

            /* Perform Xof hash */
            myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
            byte[] myOutput = new byte[pTest.theHash.Length];
            CheckMultiOutput(myDigest, myOutput);
            Assert.IsTrue(Arrays.AreEqual(pTest.theHash, myOutput), "digest mismatch");
        }

        /**
         * Run keyedTest.
         *
         * @param pTest the test
         */
        private void TestKeyed(TestDef pTest)
        {
            /* Create digest */
            Blake3Digest myDigest = new Blake3Digest();
            Blake3Parameters myParams = Blake3Parameters.Key(BLAKE3KEY);
            myDigest.Init(myParams);

            /* Try a fake update */
            myDigest.BlockUpdate(BLAKE3KEY, 0, BLAKE3KEY.Length);
            myDigest.Reset();

            /* Save just before final and reset */
            myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
            Blake3Digest mySave = (Blake3Digest)myDigest.Copy();
            myDigest.Reset();

            /* restore save and perform a standard hash */
            myDigest.Reset(mySave);
            byte[] myBase = new byte[myDigest.GetDigestSize()];
            myDigest.DoFinal(myBase, 0);
            byte[] myExpected = Arrays.CopyOf(pTest.theKeyed, myBase.Length);
            Assert.IsTrue(Arrays.AreEqual(myExpected, myBase), "baseKeyedDigest mismatch");

            /* Perform Xof hash */
            myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
            byte[] myOutput = new byte[pTest.theKeyed.Length];
            CheckMultiOutput(myDigest, myOutput);
            Assert.IsTrue(Arrays.AreEqual(pTest.theKeyed, myOutput), "keyedDigest mismatch");
        }

        /**
         * Run derivedTest.
         *
         * @param pTest the test
         */
        private void TestDerived(TestDef pTest)
        {
            /* Create digest */
            Blake3Digest myDigest = new Blake3Digest();
            Blake3Parameters myParams = Blake3Parameters.Context(BLAKE3CTX);
            myDigest.Init(myParams);

            /* Try a fake update */
            myDigest.BlockUpdate(BLAKE3KEY, 0, BLAKE3KEY.Length);
            myDigest.Reset();

            /* Save just before final and reset */
            myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
            Blake3Digest mySave = (Blake3Digest)myDigest.Copy();
            myDigest.Reset();

            /* restore save and perform a standard hash */
            myDigest.Reset(mySave);
            byte[] myBase = new byte[myDigest.GetDigestSize()];
            myDigest.DoFinal(myBase, 0);
            byte[] myExpected = Arrays.CopyOf(pTest.theDerived, myBase.Length);
            Assert.IsTrue(Arrays.AreEqual(myExpected, myBase), "baseDerivedDigest mismatch");

            /* Perform Xof hash */
            myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
            byte[] myOutput = new byte[pTest.theDerived.Length];
            CheckMultiOutput(myDigest, myOutput);
            Assert.IsTrue(Arrays.AreEqual(pTest.theDerived, myOutput), "derivedDigest mismatch");
        }

        /**
         * check multi-output.
         *
         * @param pDigest the digest
         * @param pBuffer the buffer
         */
        private void CheckMultiOutput(Blake3Digest pDigest, byte[] pBuffer)
        {
            /* Take a copy of the state */
            Blake3Digest mySave = (Blake3Digest)pDigest.Copy();

            /* Crate alternative buffer */
            int myLen = pBuffer.Length;
            byte[] myAlt = new byte[myLen];

            /* Perform standard output */
            pDigest.DoOutput(pBuffer, 0, myLen);
            pDigest.Reset(mySave);

            /* Loop creating output 5 bytes at a time */
            for (int i = 0; i < myLen; i += PARTOUT)
            {
                /* Output bytes */
                int myRead = System.Math.Min(PARTOUT, myLen - i);
                pDigest.DoOutput(myAlt, i, myRead);

                /* Around halfway through */
                if (i == PARTBREAK)
                {
                    /* Check save and reset in the middle of output */
                    Blake3Digest myTemp = (Blake3Digest)pDigest.Copy();
                    pDigest.Reset();
                    pDigest.Reset(myTemp);
                }
            }
            pDigest.Reset(mySave);
            Assert.IsTrue(Arrays.AreEqual(pBuffer, myAlt), "multiOutput mismatch");
        }

        /**
         * Test Definition.
         */
        private class TestDef
        {
            /**
             * The data length.
             */
            internal readonly int theLength;

            /**
             * The testData.
             */
            internal readonly byte[] theData;

            /**
             * The expected hash.
             */
            internal readonly byte[] theHash;

            /**
             * The expected keyedHash/
             */
            internal readonly byte[] theKeyed;

            /**
             * The expected derived Hash.
             */
            internal readonly byte[] theDerived;

            /**
             * Constructor.
             *
             * @param pLength  the length
             * @param pHash    the expected hash.
             * @param pKeyed   the expected keyed hash
             * @param pDerived the expected derived hash.
             */
            internal TestDef(int pLength, string pHash, string pKeyed, string pDerived)
            {
                /* Store parameters */
                theLength = pLength;
                theHash = Hex.Decode(pHash);
                theKeyed = Hex.Decode(pKeyed);
                theDerived = Hex.Decode(pDerived);

                /* Generate test data */
                theData = new byte[theLength];
                for (int i = 0; i < theLength; i++)
                {
                    theData[i] = (byte)(i % BLAKE3MOD);
                }
            }
        }

        [Test]
        public void TestFunction()
        {
            string resultText = Perform().ToString();

            Assert.AreEqual(Name + ": Okay", resultText);
        }
    }
}
