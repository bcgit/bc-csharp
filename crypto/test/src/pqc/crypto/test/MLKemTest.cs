...
namespace Org.BouncyCastle.Pqc.Crypto.Tests
{
    ...
    public class MLKemTest
    {
        ...
        public void TestMLKem()
        {
            ...
            Assert.True(Arrays.AreEqual(Hex.Decode(expectedPubKey), ((MLKemPublicKeyParameters)keyPair.Public).GetEncoded()));
            Assert.True(Arrays.AreEqual(Hex.Decode(expectedPrivKey), ((MLKemPrivateKeyParameters)keyPair.Private).GetEncoded()));
            ...
            ISecretWithEncapsulation secretEncap = kemGen.InternalGenerateEncapsulated(keyPair.Public, message);
            string expectedSharedSecret = "5CF38F578AC4AE95FBFED574B3D8EBF7CB1DC9074F22277360E36D775347C058";
            Assert.True(Arrays.AreEqual(Hex.Decode(expectedSharedSecret), secretEncap.GetSecret()));
            ...
            Assert.True(Arrays.AreEqual(Hex.Decode(expectedCipherText), secretEncap.GetEncapsulation()));
            ...
            Assert.True(Arrays.AreEqual(Hex.Decode(expectedSharedSecret), decryptedSharedSecret));
        }
        ...
        {
            Assert.AreEqual(256, MLKemParameters.ML_KEM_512.SessionKeySize);
            Assert.AreEqual(256, MLKemParameters.ML_KEM_768.SessionKeySize);
