...
namespace Org.BouncyCastle.Bcpg.OpenPgp.Tests
{
    ...
    {
        // Keep in sync with the cap in the (Signature|UserAttribute)SubpacketsParser classes.
        ...
        public void UserAttributeSubpacketOverCapIsRejected()
        {
            ...
            byte[] packet = BuildSubpacket(MaxSubpacketLength + 2, tag: 100);

            var parser = new UserAttributeSubpacketsParser(new MemoryStream(packet, writable: false));

            Assert.Throws<EndOfStreamException>(() => parser.ReadPacket());
        }
