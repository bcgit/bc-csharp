...
namespace Org.BouncyCastle.Tsp.Tests
{
    ...
    public class ParseTest
    {
        ...
        /// <summary>
        /// An empty/truncated stream - e.g. an empty or dropped TSA HTTP response - must surface as the declared
        /// <see cref="IOException"/> (<see cref="TspException"/> for a response), never a bare
        /// <see cref="NullReferenceException"/> escaping the throws contract.
        /// </summary>
        /// <remarks>
        /// Reading an empty stream yields a null ASN.1 object, which was passed on to the value ctor and dereferenced.
        /// </remarks>
        [Test]
        public void EmptyInputRejectedCleanly()
        {
            ...
            try
            {
                new TimeStampRequest(new MemoryStream(new byte[0]));
                Assert.Fail("TimeStampRequest(Stream) accepted empty input");
            }
            catch (IOException)
            {
                // declared contract
            ...
            {
                Assert.Fail($"TimeStampRequest(Stream) leaked {e.GetType().FullName} on empty input");
            }
            ...
            try
            {
                ...
                Assert.Fail("TimeStampRequest(byte[]) accepted empty input");
            }
            catch (IOException)
            {
                // declared contract
            ...
            {
                Assert.Fail($"TimeStampRequest(byte[]) leaked {e.GetType().FullName} on empty input");
            }

            // TimeStampResponse - empty stream
            try
            {
                new TimeStampResponse(new MemoryStream(new byte[0]));
                Assert.Fail("TimeStampResponse(Stream) accepted empty input");
            }
            catch (IOException)
            {
                // declared contract
            ...
            {
                Assert.Fail($"TimeStampResponse(Stream) leaked {e.GetType().FullName} on empty input");
            }
            ...
            try
            {
                ...
                Assert.Fail("TimeStampResponse(byte[]) accepted empty input");
            }
            catch (IOException)
            {
                // declared contract
            ...
            {
                Assert.Fail($"TimeStampResponse(byte[]) leaked {e.GetType().FullName} on empty input");
            }
            ...
            //    Assert.Fail("CmsTimeStampedData accepted empty input");
            //}
            //catch (IOException)
            //{
            //    // declared contract
            ...
            //catch (Exception e) when (!(e is AssertionException))
            //{
            //    Assert.Fail($"CmsTimeStampedData leaked {e.GetType().FullName} on empty input");
            //}
        }

        [Test]
        public void GeneralizedTime()
        {
            // Believe it or not but 'generalizedTime' contains invalid integers
            Properties.WithThreadProperty(Properties.Asn1AllowUnsafeInteger, bool.TrueString, () =>
            {
                TimeStampResponse resp = new TimeStampResponse(generalizedTime);

                Assert.AreEqual((int)PkiStatus.Granted, resp.Status, "request not rejected.");
            });
        }
        ...
        {
            TimeStampResponse resp = new TimeStampResponse(unacceptablePolicy);
        ...
        [Test]
        public void V2SigningResponseParse()
        {
            TimeStampResponse response = new TimeStampResponse(v2SigningCertResponse);
