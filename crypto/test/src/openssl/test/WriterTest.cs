...
namespace Org.BouncyCastle.OpenSsl.Tests
{
        ...
        : SimpleTest
    {
        ...
        public override string Name => "WriterTest";
        ...
        public override void PerformTest()
        {
            ...
            ImplWriteReadTest(testDsaKey);
            ImplWriteReadTests(testDsaKey, algorithms);

            ImplWriteReadTest(testRsaKey);
            ImplWriteReadTests(testRsaKey, algorithms);

            AsymmetricKeyParameter ecPriv = PrivateKeyFactory.CreateKey(testEcDsaKeyBytes);
            ImplWriteReadTest(ecPriv);
            ImplWriteReadTests(ecPriv, algorithms);

            var ecKpg = GeneratorUtilities.GetKeyPairGenerator("ECDSA");
        ...
        private void ImplWriteReadTests(AsymmetricKeyParameter akp, string[] algorithms)
        {
            foreach (string algorithm in algorithms)
            {
                ImplWriteReadTest(akp, algorithm);
            }
        }

        private void ImplWriteReadTest(AsymmetricKeyParameter akp)
        {
        ...
        private void ImplWriteReadTest(AsymmetricKeyParameter akp, string algorithm)
        {
            var passwordFinder = new TestPassword("bouncy");

            StringWriter sw = new StringWriter();
            using (var pw = new PemWriter(sw))
            {
                pw.WriteObject(akp, algorithm, passwordFinder.GetPassword(), Random);
            }

            string data = sw.ToString();

            using (var pr = new PemReader(new StringReader(data), passwordFinder))
            {
                AsymmetricCipherKeyPair kp = pr.ReadObject() as AsymmetricCipherKeyPair;
