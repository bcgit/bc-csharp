...
namespace Org.BouncyCastle.Cms.Tests
{
    ...
    {
        private static readonly byte[] TestData = Encoding.ASCII.GetBytes("Hello world!");
        ...
        public void TestWorkingData()
        {
            ...
            Assert.That(Arrays.AreEqual(uncompData, ed.GetContent()));
        }
        ...
        public void TestEach()
        {
            ...
            Assert.AreEqual(PkcsObjectIdentifiers.Data, cd.CompressedContentType);

            using (var contentStream = cd.GetContentStream())
            {
                Assert.AreEqual(PkcsObjectIdentifiers.Data, contentStream.ContentTypeOid);
            }

            Assert.That(Arrays.AreEqual(TestData, cd.GetContent()));
        }
        ...
        public void TestContentType()
        {
            ...
            Assert.AreEqual(PkcsObjectIdentifiers.SafeContentsBag, cd.CompressedContentType);

            using (var contentStream = cd.GetContentStream())
            {
                Assert.AreEqual(PkcsObjectIdentifiers.SafeContentsBag, contentStream.ContentTypeOid);
            }
        ...
        public void TestLimitUnder()
        {
            ...
            try
            {
                cd.GetContent(TestData.Length / 2);
            }
            catch (CmsException e)
            {
                Assert.That(e.InnerException is StreamOverflowException);
            }
        }
        ...
        public void TestLimitOver()
        {
            ...
            Assert.That(Arrays.AreEqual(TestData, cd.GetContent(TestData.Length * 2)));
        }
        ...
        public void TestLimitEqual()
        {
            ...
            Assert.That(Arrays.AreEqual(TestData, cd.GetContent(TestData.Length)));
        }

        private static CmsCompressedData GetStdData() => GetStdData(CmsObjectIdentifiers.Data);

        private static CmsCompressedData GetStdData(DerObjectIdentifier contentType)
        {
            CmsProcessableByteArray testData = new CmsProcessableByteArray(contentType, TestData);
